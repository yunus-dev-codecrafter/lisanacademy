...
if (!function_exists('ui_icon')) {
    ...
    function ui_icon($name, $size = 18, $class = '') {
        $icons = [
            ...
            'history'     => '<path d="M3 3v5h5"/><path d="M3.05 13A9 9 0 1 0 6 5.3L3 8"/><polyline points="12 7 12 12 17 14"/>',
    ...
    function ui_sidebar($role, $active = '') {
        if ($role === 'admin') {
            ...
            $pending_teaching = function_exists('teaching_pending_count') && isset($GLOBALS['conn'])
                ...
                : 0;
            $pending_mem = function_exists('memorization_pending_count') && isset($GLOBALS['conn'])
                ? memorization_pending_count($GLOBALS['conn'])
                : 0;
            $items = [
                ...
                'memorization' => ['memorization_students.php', 'star', 'Memorization' . ($pending_mem > 0 ? ' <span class="nav-badge">' . $pending_mem . '</span>' : '')],
        ...
        $is_hafiz = (isset($GLOBALS['conn']) && isset($_SESSION['user_id']))
            ...
            : false;
        $is_memorizer = (isset($GLOBALS['conn']) && isset($_SESSION['user_id']))
            ? student_is_memorizing($GLOBALS['conn'], (int)$_SESSION['user_id'])
            : false;
        ...
        if ($is_hafiz) {
            ...
            $items = [
                'dashboard'    => ['dashboard.php',        'grid',   'Dashboard'],
                'islamiyya'    => ['islamiyya.php',        'book-open', 'Digital Islamiyya'],
                'memorization' => ['quran_memorization.php','star',   'Qur\'an Memorization'],
                'announcements'=> ['announcements.php',    'bell',   'Announcements'],
                'feedback'     => ['feedback.php',         'chat',   'Feedback'],
