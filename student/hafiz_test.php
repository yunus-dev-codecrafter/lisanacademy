...
function startRec(i) {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(s => {
            ...
            catch (e) { alert('Could not start recording. You can upload a file instead.'); s.getTracks().forEach(t => t.stop()); return; }
            ...
            recorders[i] = rec;
            try { rec.start(1000); } catch (e) { alert('Could not start recording. Please upload a file instead.'); s.getTracks().forEach(t => t.stop()); return; }
...
function render_test_questions_readonly($questions, $test) {
    ...
    foreach ($questions as $idx => $q) {
        ...
        if (!empty($q['audio_file'])) {
            $__ext = strtolower(pathinfo($q['audio_file'], PATHINFO_EXTENSION));
            $__url = '../uploads/hafiz_test_audio/' . rawurlencode($q['audio_file']);
            if (in_array($__ext, ['mov', 'mp4', 'm4v', '3gp', '3gpp'], true)) {
                $html .= '<br><video controls preload="none" playsinline style="width:100%;margin-top:6px;border-radius:12px;background:#000;" src="' . htmlspecialchars($__url) . '"></video>';
            } else {
